...
int main(int argc, char **argv) {
    ...
    char temp, buf[4000], *execvp_argv[1], compbuf[4000], decompbuf[4000];
    int opt, c, sockfd, newsockfd, portno, i, k, ret, end, input, port_given, to_child_pipe[2], from_child_pipe[2]; 
    socklen_t clilen; 
            ...
            if (fds[k].revents & POLLIN) {
              ...
              if(k == 0) {
                ...
                if(c) {
                    ...
                    sockettoshell.avail_in = (uInt)input; 
                    ...
                    if(inflateInit(&sockettoshell) < 0) {
                        fprintf(stderr, "Cannot initialize compression");
                        exit(1);
                    }
                    do {
                    ...
                    input = (int)sizeof(decompbuf) - sockettoshell.avail_out; 
                    inflateEnd(&sockettoshell);
                }
                for (i = 0; i < input; i++) {
                    /* C-d */
                    if (buf[i] == '\004') {
              ...
              /* reading from child pipe (shell) */
                ...
                memset(buf, 0, 4000); 
                input = 0; 
                input = read (from_child_pipe[0], buf, 4000);
                if(input < 0) {
                  fprintf(stderr, "Read error: %s\n", strerror(errno));
                  exit(1);
                }
                ...
                if(c) {
                    ...
                    write(newsockfd, compbuf, input); 
                    deflateEnd(&shelltosocket);                     
                }
              }
            }
